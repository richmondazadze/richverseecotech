...
const ImpactStat = ({ value, label, sublabel, delay = 0 }) => {
  const [count, setCount] = useState(0);
  useEffect(() => {
    let start = 0;
    const end = typeof value === 'number' ? value : parseInt(value.replace(/\D/g, ''));
    if (start === end) return;
  ...
  return (
    <motion.div whileHover={{ scale: 1.04 }} className="bg-blue-50 rounded-2xl shadow-lg p-8 flex flex-col items-center text-center animate-fade-in-up">
...
const aboutFaqs = [
  ...
  },
  {
    ...
    answer: (
      <>
        We welcome partnerships! <a href="/contact" className="text-blue-600 underline hover:text-blue-800">Contact us</a> to discuss collaboration opportunities.
      </>
    ),
  },
  {
